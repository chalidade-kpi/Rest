...
class EInvo{
	...
	private static function getHdrInvAR($arr){
		$hdrRequest = DB::connection("omuster")->table($arr["config"]["head_table"])->where($arr["config"]["head_no"], $arr['nota']['nota_req_no'])->first();
		...
		return $hdr = '"header": {
							...
							"attribute15": "",
							"interfaceHeaderAttribute1": "'.$arr['nota']['nota_no'].'",
							"interfaceHeaderAttribute2": "'.$arr['nota']['nota_req_no'].'",
							"interfaceHeaderAttribute3": "'.$hdrRequest[$arr["config"]["head_vin"]].'",
							"interfaceHeaderAttribute4": "'.$arr['nota']['nota_vessel_name'].'",
							"interfaceHeaderAttribute5": "",
							"interfaceHeaderAttribute6": "",
							"interfaceHeaderAttribute7": "",
							"interfaceHeaderAttribute8": "",
							"interfaceHeaderAttribute9": "'.$hdrRequest[$arr["config"]["head_vout"]].'",
	...
	private static function getDtlInvAR($arr){
		$lines = '';
		$getNotaDtl = DB::connection('eng')->table('V_TX_TEMP_TARIFF_DTL_NPKS')->where('booking_number',$arr['nota']['nota_req_no'])->get();
		foreach ($getNotaDtl as $list) {
			$condMemo = [
				"BRANCH_ID" 	=> $arr['nota']['nota_branch_id'],
				"BRANCH_CODE" => $arr['nota']['nota_branch_code'],
				"GROUP_TARIF" => $list->group_tariff_id
			];
			$memoline 			= DB::connection('mdm')->table('TM_MEMOLINE')->where($condMemo)->first();
			$lineattribut5	= $list->cont_size." " .$list->cont_type." ".$list->cont_status." N";
			$lineAttribut6  = null;

			if ($list->group_tariff_id == 10)
				$lineAttribut6	= $list->masa1 + $list->masa2;
			...
			$lines				 .= '
			{
				...
				"lineId": null,
				"lineNumber": "'.$line.'",
				"description": "'.$list->group_tariff_name.'",
				"memoLineId": null,
				"glRevId": null,
				"lineContext": "'.$arr['nota']['nota_sub_context'].'",
				"taxFlag": "Y",
				"serviceType": "'.$memoline->memoline.'",
				"eamCode": "",
				"locationTerminal": "",
				"amount": "'.$list->dpp.'",
				"taxAmount": "'.$list->ppn.'",
				...
				"interfaceLineAttribute1": " / '.$arr['nota']['nota_no'].'",
				"interfaceLineAttribute2": null,
				"interfaceLineAttribute3": "",
				"interfaceLineAttribute4": "'.$list->qty.'",
				"interfaceLineAttribute5": "'.$lineattribut5.'",
				"interfaceLineAttribute6": "'.$lineAttribut6.'",
				"interfaceLineAttribute7": "'.$list->tariff.'",
				"interfaceLineAttribute8": "",
				"interfaceLineAttribute9": "",
				"interfaceLineAttribute10": "'.$list->cont_size.'",
				"interfaceLineAttribute11": "'.$list->cont_type.'",
				"interfaceLineAttribute12": "'.$list->cont_status.'",
				"interfaceLineAttribute13": "N",
				"interfaceLineAttribute14": null,
